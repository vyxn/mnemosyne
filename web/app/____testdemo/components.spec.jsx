...
import { BeerListContainer } from './components.jsx';
import { InputArea, BeerList } from './components.jsx';
...
describe('<InputArea/>', () => {
  ...
  it('should clear input after click', () => {
    ...
    wrapper.setState({ text: 'SomeText' });
    const inputText = wrapper.find('input');
    // inputText.text('Sam Adams');
    const addButton = wrapper.find('button');
    addButton.simulate('click');
    expect(wrapper.state('text')).to.equal('');
  });

  it('should execute onSubmit when pressing enter', () => {
    const onSubmitSpy = spy();
    const wrapper = mount(<InputArea onSubmit={onSubmitSpy}/>);
    wrapper.setState({ text: 'SomeText' });
    const inputText = wrapper.find('input');
    inputText.simulate('keyDown', { keyCode: 13 });
    expect(onSubmitSpy.calledOnce).to.equal(true);
    expect(onSubmitSpy.calledWith('SomeText')).to.equal(true);
  });
});
